...
- (id)initWithTitle:(NSString *)aTitle options:(NSArray *)aOptions xy:(CGPoint)point size:(CGSize)size isMultiple:(BOOL)isMultiple
{
    ...
    if (self = [super initWithFrame:rect])
    {
        ...
        _kTableView.separatorColor = [UIColor colorWithWhite:0.3 alpha:.2];
        ...
        if (isMultipleSelection) {
            ...
            [btnDone setFrame:CGRectMake(DROPDOWNVIEW_SCREENINSET + 200,DROPDOWNVIEW_SCREENINSET +10 ,
                                         80, 30)];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if([self.arryData containsObject:indexPath]){
        imgarrow.frame=CGRectMake(250,(cell.frame.size.height-20)/2, 20, 20);
...
-(void)setSelecteditem:(NSMutableArray *)arr;
{
    self.arryData=[[NSMutableArray alloc]init];
    for (int k=0; k<arr.count; k++) {

        
        if([_kDropDownOption containsObject:[arr objectAtIndex:k]]){
            
            NSInteger inde=[_kDropDownOption indexOfObject:[arr objectAtIndex:k]];
            
            [self.arryData addObject:[NSIndexPath indexPathForRow:inde inSection:0]];
        }
